...
pygame.display.set_caption("Snake devours!")
     ...
     def update(self):
         ...
         for square in self.body:
            if snake.head.x == square.x and snake.head.y == square.y:
             self.dead = True
            if snake.head.x not in range(0, SW) or snake.head.y not in range(0, SH):
             self.dead = True
...
while True:
    ...
    head_image = pygame.image.load("assets/shead.png").convert_alpha()
    head_image = pygame.transform.scale(head_image, (BLOCK_SIZE, BLOCK_SIZE))
    screen.blit(head_image, snake.head)
